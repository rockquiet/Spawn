...
public class UpdateChecker {
    ...
    public UpdateChecker(Spawn plugin) {
        if (Spawn.isFolia)
            Bukkit.getAsyncScheduler().runNow(plugin, run -> {
                try {
                    ...
                    if (pluginVersion.contains("SNAPSHOT")) {
                        plugin.getLogger().info(
                                "You are running a development build, please report any bugs on the project's GitHub.");
                        plugin.getLogger()
                                .info("Latest release version: " + latest + ", you are using: " + pluginVersion);
                        return;
                    }
                    ...
                    if (compare > 0) {
                        ...
                        plugin.getLogger().warning(
                                "You are running a newer version of the plugin than released. If you are using a development build, please report any bugs on the project's GitHub.");
                    }
                } catch (IOException e) {
